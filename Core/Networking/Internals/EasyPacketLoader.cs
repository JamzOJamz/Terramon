...
internal sealed class EasyPacketLoader : ModSystem
{
    ...
    internal static void AddHandler<T>(HandleEasyPacketDelegate<T> handler) where T : struct, IEasyPacket<T>
    {
        ...
        if (!HandlerByPtr.ContainsKey(ptr)) HandlerByPtr.Add(ptr, null);
    ...
    {
        var ptr = typeof(T).TypeHandle.Value;
        if (!HandlerByPtr.ContainsKey(ptr)) return;
    ...
    public override void Load()
    {
                     ...
                     .OrderBy(m => m.Name, StringComparer.InvariantCulture))
        {
            ...
            foreach (var type in loadableTypes
                         .Where(t => t.IsValueType && !t.ContainsGenericParameters &&
                                     t.GetInterface(EasyPacketFullName) != null)
                         .OrderBy(t => t.FullName, StringComparer.InvariantCulture))
                RegisterPacket(mod, type);

            foreach (var type in loadableTypes
                         .Where(t => t.IsValueType && !t.ContainsGenericParameters &&
                                     t.GetInterface(EasyPacketHandlerFullName) != null)
                         .OrderBy(t => t.FullName, StringComparer.InvariantCulture))
    ...
    private void RegisterPacket(Mod mod, Type type)
    {
        ...
        if (instance == null) throw new Exception($"Failed to register easy packet type: {type.Name}.");
    ...
    private void RegisterHandler(Mod mod, Type type)
    {
        ...
        var instance = (IEasyPacketHandler)Activator.CreateInstance(
            typeof(EasyPacketHandler<,>).MakeGenericType(type,
                type.GetInterface(EasyPacketHandlerFullName)!.GetGenericArguments()[0]), true);
        if (instance == null) throw new Exception($"Failed to register easy packet type: {type.Name}.");
