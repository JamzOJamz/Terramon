...
public abstract class CustomPreviewTile : ModTile
{
    ...
    private static void ILCustomPreviewDrawing(ILContext il)
    {
        try
        {
            ...
            c.EmitDelegate<Func<SpriteBatch, Texture2D, Vector2, Rectangle?, Color, TileObjectPreviewData, bool>>(
                (sb, t, p, r, c, top) =>
                {
                    var modTile = TileLoader.GetTile(top.Type);
    ...
    public virtual bool PreDrawPlacementPreview(SpriteBatch sb, TileObjectPreviewData data, Texture2D texture,
